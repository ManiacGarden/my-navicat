SELECT app_id, name,
       count(*),
       sum(increase60) / count(*) as avg_increase
from (select *,
             0.02631 - 0.26687 * roi1 + 0.42143 * roi2 + 0.01825 * roi3 - 0.24494 * roi4 - 1.02253 * roi5 +
             1.22622 * roi6 + 0.13677 * roi7 - 1.31363 * roi15 + 1.98775 * roi30         as roi_pred60,
             0.02631 - 0.26687 * roi1 + 0.42143 * roi2 + 0.01825 * roi3 - 0.24494 * roi4 - 1.02253 * roi5 +
             1.22622 * roi6 + 0.13677 * roi7 - 1.31363 * roi15 + 1.98775 * roi30 - roi60 as increase60
      from (SELECT r.app_id,
            ...
            where 1
            -- and r.app_id = 'wxef3d935be4e18729'
-- and date_at >= '2025-04-02'
            GROUP BY r.app_id, date_at
            having max(ad_cost > 1000)
               and MAX(CASE WHEN date_after = 59 THEN ad_roi END) > 0
            order by r.app_id asc, date_at asc) as roi1
      order by increase60 asc
			) as roi2 
			where   0.02631 - 0.26687 * roi1 + 0.42143 * roi2 + 0.01825 * roi3 - 0.24494 * roi4 - 1.02253 * roi5 +
             1.22622 * roi6 + 0.13677 * roi7 - 1.31363 * roi15 + 1.98775 * roi30 - roi60 < 0
group by app_id
order by count(*) desc