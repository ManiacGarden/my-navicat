...
on g.app_id = w.app_id
    ...
    left join (
        ...
        -- 所有游戏多日
            where date_at >= '2025-04-14' and date_at <= '2025-04-19'
            group by app_id
        所有游戏多日总和
--         where date_at >= '2025-04-14' and date_at <= '2025-04-19'
        -- 单日所有游戏总和
--         where date_at = '2025-04-19'
--     单个游戏多日总和
--         where date_at >= '2025-04-14' and date_at <= '2025-04-19' and app_id='wx2bd39bc6b432744f'
-- 分游戏
...
-- 		) gr on 1 = 1
		left join (
			select tmp.app_id,
			-- 对于每一行，如果 income1 都不为 0 或 null，才算 sum(income1) / sum(ad_cost)，否则 直接记为 0。
			CASE 
				WHEN SUM(CASE WHEN tmp.income1 IS NULL OR tmp.income1 = 0 THEN 1 ELSE 0 END) = 0 
				THEN SUM(tmp.income1) / SUM(ad_cost)
				ELSE 0
			END AS roi1,
			CASE 
				WHEN SUM(CASE WHEN tmp.income3 IS NULL OR tmp.income3 = 0 THEN 1 ELSE 0 END) = 0 
				THEN SUM(tmp.income3) / SUM(ad_cost)
				ELSE 0
			END AS roi3,
			CASE 
				WHEN SUM(CASE WHEN tmp.income7 IS NULL OR tmp.income7 = 0 THEN 1 ELSE 0 END) = 0 
				THEN SUM(tmp.income7) / SUM(ad_cost)
				ELSE 0
			END AS roi7
			from 
					(
						...
						from tq_wx_amp_roi
		-- 					 单个游戏多日总和
-- 						where app_id ='wx2bd39bc6b432744f' and date_at >= '2025-04-14' and date_at <= '2025-04-19' GROUP BY date_at, app_id
		-- 					 所有游戏多日总和
-- 						where date_at >= '2025-04-14' and date_at <= '2025-04-19' GROUP BY date_at, app_id
						-- 所有游戏多日
						where date_at >= '2025-04-14' and date_at <= '2025-04-19' GROUP BY date_at, app_id
-- 单日所有游戏总和
--   where date_at = '2025-04-19' GROUP BY date_at, app_id
					) tmp
			group by tmp.app_id
					-- 单日多个游戏
		) r on w.app_id = r.app_id
...
-- 所有游戏多日
--   and w.date >= '2025-04-14' and w.date <= '2025-04-19'
-- group by w.app_id
-- order by active_num desc
-- 所有游戏多日总和
--   and w.date >= '2025-04-14' and w.date <= '2025-04-19'
-- 单日所有游戏总和
  and w.date = '2025-04-19'
-- 单个游戏多日总和
--   and w.date >= '2025-04-14' and w.date <= '2025-04-19'
