SELECT 
		   ...
		   -- 发行人任务金
			 -- 投放广告收入（分成前） * 0.05
		   p.total_income * 0.05 as advertiser_encourage,
		 ...
		 left join (
				SELECT
					ad.id id, ad.app_id, ad.date_time, ad.video_reward, ad.live_reward, ad.ps,
					sum(expense) expense,
...
WHERE t.date_time = '2025-07-05' GROUP BY t.app_id, t.date_time
				order by ad.app_id, ad.date_time desc
		 ) t on t.app_id = p.app_id and t.date_time = p.date left join tt_game_analysis_data a on t.app_id = a.app_id and t.date_time = a.date join game g on g.app_id = t.app_id WHERE t.date_time = '2025-07-05' ORDER BY expense_cpa DESC